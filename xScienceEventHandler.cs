...
namespace ScienceChecklist
{
    public class xScienceEventHandler
    {
        ...
        private void GameStateSave(ConfigNode C)
        {
            ...
            //WarezCrawler 2019-04-16 - Problems with update of experiments when changing SOI
            Debug.Log("[XS-GTI] Saving Game..... And update all()");
            ScienceContext.LoadStatus.OnboardScienceUnloadedVessels = false;
            ScienceContext.LoadStatus.Experiments = false;
        ...
        {
            Debug.Log("[XS-GTI] ScienceRecieved()");
            //ScienceContext.updateScienceSubjects = true;
            ScienceContext.LoadStatus.ScienceSubjects = false;
        ...
        private void DominantBodyChange(GameEvents.FromToAction<CelestialBody, CelestialBody> Data)
        {
            ...
            //ScienceContext.ExperimentCacheLoaded = false;       //Reset gamecache when changing body
            ScienceContext.LoadStatus.ScienceSubjects = false;
            ScienceContext.LoadStatus.ExperimentCache = false;
            ScheduleExperimentUpdate(true);
        }
